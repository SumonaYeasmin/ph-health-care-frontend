...
export const useSpecialtySelection = ({
        ...
        if (isEdit && doctor?.doctorSpecialties) {
            return (
                doctor?.doctorSpecialties
                    ?.map((ds) => {
                        // Fallbacks: specialitiesId or specialties.id (matching types/doctor.interface.ts)
                        return (
                            (ds as any)?.specialitiesId || ds?.specialties?.id || null
                        );
                    })
    ...
    const getNewSpecialties = (): string[] => {
        ...
        const originalIds =
            doctor?.doctorSpecialties
                ?.map(
                    (ds) => (ds as any)?.specialitiesId || ds?.specialties?.id || null
                )
                ?.filter((id): id is string => !!id) || [];
