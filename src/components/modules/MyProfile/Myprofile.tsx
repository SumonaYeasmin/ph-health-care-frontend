...
import { updateMyProfile } from "@/src/services/auth/auth.service";
...
// retrieves the profile photo URL based on the user's role.
// Admins, Doctors, and Patients store their photos in their respective sub-objects.
...
//  Retrieves the role-specific profile details object from user info.
//  Returns the nested admin, doctor, or patient data object depending on user role.
  ...
  // Cache the derived profile photo URL and profile details object
  const profilePhoto = getProfilePhoto();
  const profileData = getProfileData();

 // handles profile picture file selection and updates the preview image state
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        // Update local preview state with the base64 string
...
// handles the profile form submission.
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Collect all input fields and files from the form
    const formData = new FormData(e.currentTarget);

    // Execute the async server action within a transition to manage loading state
    startTransition(async () => {
      const result = await updateMyProfile(formData);

      if (result.success) {
        setSuccess(result.message);
        setPreviewImage(null); // Reset file selection preview on success
